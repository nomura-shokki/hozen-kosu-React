...
class Open_pages(TestCase):
  ...
  def test_history_list(self):
    # URL定義
    url = reverse('history_list', args=[1])
    # URLに対してGETリクエスト送信
    response = self.client.get(url)

    # リクエストのレスポンスステータスコードが200(OK)であることを確認
    self.assertEqual(response.status_code, 200)
    # レスポンスコンテンツに「編集履歴一覧」という文字列が含まれていることを確認
    self.assertContains(response, '編集履歴一覧')
  ...
  def test_history_list(self):
    # URL定義
    url = reverse('history_delete', args=[self.Operation_history.id])
    # URLに対してGETリクエスト送信
    response = self.client.get(url)

    # リクエストのレスポンスステータスコードが200(OK)であることを確認
    self.assertEqual(response.status_code, 200)
    # レスポンスコンテンツに「編集履歴詳細」という文字列が含まれていることを確認
    self.assertContains(response, '編集履歴詳細')
