...
class Page_jump(TestCase):
  ...
  def test_team_detail_team_kosu_jump(self):
    # 班員工数詳細から来たことを明示
    self.session['before_page'] = 'team_list'
    # 工数詳細ページにアクセス
    response = self.client.get(reverse('team_detail', args=[self.Business_Time_graph.id]))
    self.assertEqual(response.status_code, 200)

    # HTMLに含まれるボタンが正しく設定されているかを確認
    self.assertContains(response, '<a href="' + reverse('team_kosu', args=[1]) + '" class="text-danger">班員工数一覧へ</a>', html=True)
    # ボタンを押すシミュレーション
    response = self.client.get(reverse('team_kosu', args=[1]))
  ...
  def test_team_detail_team_kosu_jump(self):
    ...
    # HTMLに含まれるボタンが正しく設定されているかを確認
    self.assertContains(response, '<a href="' + reverse('class_list') + '" class="text-danger">工数入力状態可否一覧へ</a>', html=True)
    # ボタンを押すシミュレーション
